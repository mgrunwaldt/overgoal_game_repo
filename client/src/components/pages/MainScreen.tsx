...
export default function MainScreen() {
    ...
    const teamIdToFind = player?.selected_team_id && player.selected_team_id > 0 ? player.selected_team_id : passedTeamId;
    ...
    if (isScreenLoading) {
        return (
            <div className="h-screen w-screen bg-cover bg-center bg-no-repeat flex flex-col items-center justify-center p-4" style={{ backgroundImage: "url('/Screens/Main/Background.png')" }}>
                <div className="text-center space-y-6">
                    <Loader2 className="w-16 h-16 animate-spin text-cyan-400 mx-auto" />
                    <div className="text-cyan-300 font-bold text-xl">Loading Player Data...</div>
                    <div className="text-cyan-400 text-sm">Please wait while we fetch your information</div>
                </div>
            </div>
        );
    }
