...
const MatchDecision: React.FC<MatchDecisionProps> = ({ isOpen, onClose, matchActionType = 'OpenPlay', onDecisionSelect, eventText, playerType = 'striker', isShowingResult, isLoadingResult, resultText, onContinue, teamNumber = 0, isChainedActionResult = false, onCaptureMinuteEvents, currentMinute, allTimelineEvents, onStoreUIText }) => {
    ...
    useEffect(() => {
        if (isOpen && onCaptureMinuteEvents && currentMinute !== undefined && allTimelineEvents && !hasCapturedEventsRef.current) {
            console.log("📋 MatchDecision opened - capturing events for minute:", currentMinute);
    ...
    useEffect(() => {
        if (isOpen && !isShowingResult && !isLoadingResult && eventText && currentMinute !== undefined && onStoreUIText && !hasStoredActionTextRef.current) {
    ...
    }, [isOpen, isShowingResult, isLoadingResult, eventText, currentMinute, onStoreUIText]);

    // 🎯 NEW: Store result text ONLY ONCE when modal shows result
    useEffect(() => {
        if (isOpen && isShowingResult && resultText && currentMinute !== undefined && onStoreUIText && !hasStoredResultTextRef.current) {
    ...
    const getDecisionsByActionType = (actionType: MatchActionType, teamNumber: number): string[] => {
        switch (actionType) {
            case 'OpenPlay':
    ...
    const getEventImage = (actionType: MatchActionType, playerType: string): string => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="w-full max-w-md mx-auto flex flex-col items-center justify-center relative">
                ...
                {/* Player Display */}
                <div
                    className="w-[380px] h-[450px] bg-contain bg-no-repeat bg-center flex items-center justify-center p-4"
                >
                    <img 
                        src={eventImage} 
                ...
                >
                    ...
                    <>
                        <div className="flex flex-col items-center justify-center">
                            ...
                            <p className="text-cyan-200 text-center text-xl font-semibold" style={{ textShadow: '0 0 8px rgba(34,211,238,0.7)' }}>
                    ...
                    // 🎯 RESULT DISPLAY MODE
                    <>
                        <p className="text-cyan-200 text-center text-xl mb-8 leading-relaxed font-semibold" style={{ textShadow: '0 0 8px rgba(34,211,238,0.7)' }}>
                            {resultText}
                        </p>
