...
const MatchComponent = () => {
  ...
  useEffect(() => {
    ...
    if (isShowingActionResult || isLoadingActionResult || isWaitingForUserAction || isWaitingForMatchContinuation || isProcessingMatchContinuation) {
      console.log("⏸️ Not initializing - showing action results, loading, waiting for user action, or waiting/processing match continuation");
      return;
    }
    ...
    if (matchTimelineEvents && matchTimelineEvents.length > 0) {
      ...
      if (initializedTimelineEventsRef.current === timelineEventsKey && !isReturningFromAction) {
        console.log("⏭️ Already initialized with these timeline events, skipping...");
        return;
      }
      
      if (isReturningFromAction) {
        console.log("🔄 Returning from action - restarting timer with existing timeline events");
      } else {
        console.log("✅ Timeline events available, initializing for the first time with key:", timelineEventsKey);
        initializedTimelineEventsRef.current = timelineEventsKey;
      }
    ...
    if (timerRef.current) {
      ...
      console.log("📋 Timeline events order check:", sortedEvents.map(e => `${e.minute}'[${e.event_id}]`).join(' → '));
      ...
      if (!isReturningFromAction) {
        ...
        console.log("📊 Updated timeline events available for timer:", sortedEvents.length);
      }
      
      // Start the timeline-driven timer
      if (isReturningFromAction) {
        console.log("🎬 Restarting timeline-driven timer from current minute...");
        startTimelineDrivenTimer(sortedEvents, currentMinute);
      } else {
        console.log("🎬 Starting timeline-driven timer with existing events...");
  ...
  }, [matchId, isShowingActionResult, isLoadingActionResult, isWaitingForUserAction, isWaitingForMatchContinuation, isProcessingMatchContinuation, matchTimelineEvents]); // 🎯 FIX: Added matchTimelineEvents, user action, and match continuation dependencies
  ...
  const startTimelineDrivenTimer = (timelineEvents: MatchTimelineEvent[], startingMinute?: number) => {
    ...
    console.log("📊 Timer will process", timelineEvents.length, "total timeline events");
    ...
    console.log("🎮 User action events:", userActionEvents.map(e => ({
      id: e.event_id,
      minute: e.minute,
      action: e.action,
    ...
    console.log("📋 Timeline events with player_participates field:", timelineEvents.map(e => ({
      event_id: e.event_id,
      minute: e.minute,
      action: e.action,
      team: e.team,
    ...
    console.log(`📊 Starting with ${processedIds.size} already processed events`);
    ...
    timerRef.current = setInterval(() => {
      ...
      console.log(`⏰ Internal timer advancing to minute ${newMinute} (display timer will update after events)`);
      ...
      if (eventsThisMinute.length > 0) {
        console.log(`📝 Found ${eventsThisMinute.length} events for minute ${newMinute}:`, eventsThisMinute.map(e => `[${e.event_id}]${e.action}`));
        console.log(`📊 Already processed event IDs:`, Array.from(processedIds));
        console.log(`📊 Available timeline events minutes:`, currentTimelineEvents.map(e => e.minute).sort((a,b) => a-b));
        console.log(`📊 Using ${currentTimelineEvents.length} total timeline events (${timelineEvents.length} original + ${Math.max(0, currentTimelineEvents.length - timelineEvents.length)} new)`);
        ...
        console.log(`🎮 User action required for minute ${newMinute}: ${requiresUserAction}`);
        ...
        console.log(`⏰ Match continuation required for minute ${newMinute}: ${requiresMatchContinuation}`);
        ...
        if (requiresUserAction) {
          ...
          if (userEvent) {
            ...
            console.log(`🕐 Display timer updating to ${newMinute}' for user action`);
      setCurrentMinute(newMinute);
          ...
          if (matchEvent) {
            console.log("⏰ Setting pending match continuation event:", matchEvent);
            setPendingMatchEvent(matchEvent);
            setIsWaitingForMatchContinuation(true);
            
            // 🎯 Update display timer when match continuation starts
            console.log(`🕐 Display timer updating to ${newMinute}' for match continuation`);
            setCurrentMinute(newMinute);
          ...
          console.log("⚽ Processing non-interactive events for minute", newMinute);
          ...
          console.log("📋 Events for minute sorted by event_id:", sortedEventsThisMinute.map(e => ({
            id: e.event_id,
            minute: e.minute,
            action: e.action
          })));
          ...
          const processNextEvent = () => {
            if (eventIndex >= sortedEventsThisMinute.length) {
              ...
              console.log(`🕐 Display timer updating to ${newMinute}' after events completed`);
              setCurrentMinute(newMinute);
              // Only clear processing state when ALL events for this minute are done
              setIsProcessingEvent(false);
              
              // 🎯 RESTART TIMER after all events are processed
              console.log("🎬 Restarting timer after event processing completed");
              startTimelineDrivenTimer(currentTimelineEvents, newMinute);
              return;
            }
            
            const evt = sortedEventsThisMinute[eventIndex];
            console.log(`📝 Processing event ${eventIndex + 1}/${sortedEventsThisMinute.length}: ${evt.event_id}`);
  ...
  const checkForChainedAction = (timelineEvents: MatchTimelineEvent[]): MatchTimelineEvent | null => {
    console.log("🔗 Checking for chained actions in timeline events:", timelineEvents.length);
    ...
    console.log("🎯 Events in current minute", currentMinute, ":", eventsInCurrentMinute.length);
    ...
    if (!resultEvent.player_participates) {
      console.log("📝 Final result has player_participates: false - NO chained action");
      return null;
    }
    
    // 🎯 Only if final result has player_participates: true, check for chained action
    console.log("🎯 Final result has player_participates: true - checking for chained action");
    ...
    if (resultEvent.event_id !== pendingUserActionEvent.event_id) {
      console.log("🔗 NEW chained action detected! Event ID:", resultEvent.event_id);
      return resultEvent;
    }
  ...
  const handleActionSelected = async (matchDecisionEnum: number) => {
    ...
    try {
      ...
      if (result) {
        ...
        if (hasGoal) {
          console.log("🎉 Goal detected in action result - triggering score blinking");
          triggerScoreBlinking();
        }
        ...
        if (chainedActionEvent) {
          ...
          setTimeout(() => {
            console.log("🔄 Transitioning from result to new chained action - timer stays stopped");
          ...
          console.log("🔄 Switched to result display mode with actual result - timer stopped");
        }
      } else {
  ...
  const handleMatchContinuation = async () => {
    ...
    if (!pendingMatchEvent || !isWaitingForMatchContinuation) {
      console.log("❌ Cannot process match continuation - no pending match event");
      return;
    }
    ...
    try {
      ...
      if (result) {
        ...
        console.log("🔄 Match continuation completed - timer will restart automatically");
      } else {
        ...
        if (pendingMatchEvent.match_end) {
          console.log("🏁 Match ended (no result data) - auto-navigating to match end screen");
          navigate(`/match-end/${matchId}`);
      ...
      if (pendingMatchEvent.match_end) {
        console.log("🏁 Match end failed but navigating anyway to avoid getting stuck");
        navigate(`/match-end/${matchId}`);
        return;
      }
    ...
    if (storedResultText && pendingUserActionEvent) {
      ...
      console.log("✅ Marking current user action as processed:", pendingUserActionEvent.event_id);
      ...
      if (hasGoal) {
        ...
        if (goalEvent) {
          ...
          const goalText = `${pendingUserActionEvent.minute}' - 🎉⚽ GOOOOOAL! ${goalEvent.team_scored ? "🔥 YOUR TEAM SCORES! 🔥" : "💀 OPPONENT SCORES! 💀"} ⚽🎉`;
      ...
      if (nextUserActionEvent) {
        ...
        console.log("✅ MARKED chained action as processed to prevent duplication:", nextUserActionEvent.event_id);
        ...
        console.log("🎯 Chained event added to timeline, waiting for user decision");
     ...
     console.log("🎬 Timer will restart automatically when isShowingActionResult becomes false...");
  };
  ...
  const captureDecisionMinuteEvents = (minute: number) => {
    ...
    console.log("📋 Events captured for minute", minute, ":", eventsForMinute.map(e => ({
      event_id: e.event_id,
      action: e.action,
  ...
  const applyStoredDecisionMinuteEvents = () => {
    ...
    console.log("📋 Applying stored decision minute UI texts for minute:", decisionMinute);
    ...
    setDisplayedEvents(prev => {
      ...
      console.log("📋 Adding stored UI events from modal:", storedUIEvents.map(e => e.text));
    ...
    <div className="relative min-h-screen">
              ...
              <ul className="text-white space-y-1 text-lg font-sans font-normal tracking-wide">
                {displayedEvents.map((event, index) => (
                  <MatchEventIten
                    ...
                    team={event.team}
                    onPlay={() => {
                      console.log("🎮 PLAY button clicked on event:", event.text);
      ...
      <MatchDecision 
        ...
        onDecisionSelect={async (decisionIndex: number) => {
          ...
          console.log(`🎮 Decision selected: index ${decisionIndex} → enum ${matchDecisionEnum} for action type: ${actionType}, team: ${teamNumber}`);
